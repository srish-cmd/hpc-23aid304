...
#
# A pass here means the public tests pass. The grader also runs hidden tests, a speedup measurement
# so a clean run here is necessary but not sufficient.
set -uo pipefail
cd "$(dirname "$0")"

SRC="${1:-starter/transpose.c}"
[ -f "$SRC" ] || SRC="transpose.c"
[ -f "$SRC" ] || { echo "no source file found (looked for starter/transpose.c and transpose.c)"; exit 1; }
...
echo "==> compiling $SRC"
gcc -O2 -std=c11 -Wall -Wextra -Werror -I"$HARNESS" -Iinclude "$SRC" tests/public.c -o .selfcheck -lm || exit 1
